...
class CozeBot(Bot):
        ...
        if context and context.type:
            if context.type == ContextType.TEXT:
                ...
                if query == "#清除记忆":
                    ...
                    reply_content,err = self.reply_text(session)
                    if err is not None:
                        logger.error("[COZE] reply error={}".format(err))
                        return Reply(ReplyType.ERROR, "我暂时遇到了一些问题，请您稍后重试~")
                    logger.info(f"[COZE] response={reply_content['content']}")
                    logger.debug(
    ...
    def _get_completion_content(self, response):
    ...
    def _get_payload(self, user: str, query: str, chat_history):
        return {
    ...
    def reply_text(self, session: CozeSession, retry_count=0):
        try:
